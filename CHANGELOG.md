...
- **非交易时段客户端反复重订阅**。推送静默重放是为"盘中静默 = 订阅丢了"设计的，但夜间静默
  才是常态：客户端每 N 个心跳重订阅一次（实测约 45 秒一次，3.3 小时里 503 条审计记录占 442 条），
  每次重订阅 bump 代次，下游把新代次读成"数据有缺口"。现在 `quote_keepalive` 回报 `known`：
  `known=True` + 无推送 = 行情安静（夜间、午休、停牌），不是故障；`known=False` = 订阅表被重置，
  客户端立即重订阅。只有服务端不回报该字段时静默规则才做决定，且每触发一次探测间隔翻倍（上限
  `max_silence_replay_backoff`，默认 16 倍）。

港股通：同一个账号几种类型，`BIGQMT_ACCOUNT_TYPE` 可写列表，客户端 `StockAccount` 的类型随请求传到服务端；延迟报告用 0.3.53 稳态重测四种组合，并修正探针的相位锁死和回放窗口两处偏差。

### 变更

- dev extra 补 `pandas`（17 个测试文件 import 它，部分在模块顶层）和自引用的
  `[redis,mysql,msgpack]`（测试要真的构造这些客户端）；运行时不依赖它们。

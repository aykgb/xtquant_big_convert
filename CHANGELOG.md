...
修掉非交易时段客户端反复重订阅的问题。83f3f86 加的推送静默重放是为"盘中静默 = 订阅丢了"
设计的，但夜间静默才是常态，客户端每 N 个心跳重订阅一次（实测约 45 秒一次，3.3 小时里
503 条审计记录占 442 条），每次重订阅 bump 代次，下游把新代次读成"数据有缺口"。

### 修复

- **`quote_keepalive` 回报 `known`**：订阅表在服务端，由它直接回答"这个订阅还在不在"。
  `known=True` + 无推送 = 行情安静（夜间、午休、停牌），不是故障；`known=False` = 订阅表
  被重置，客户端立即重订阅。不回报该字段的老服务端不受影响，客户端退回旧的静默启发式。
- **静默重放只在无应答时生效，且指数退避**：只有服务端不回报 `known`（0.3.42 之前）时
  静默规则才做决定，且每触发一次探测间隔翻倍（上限 `max_silence_replay_backoff`，默认 16
  倍），整夜安静的会话会安静下来，而不是每 N 个心跳重订阅到天亮。

### 变更

- dev extra 补 `pandas`：17 个测试文件 import pandas（部分在模块顶层），不声明的话
  `--extra dev pytest` 按原样跑有 89 个 ModuleNotFoundError，真正的失败被埋在里面。
  运行时不依赖它，只有测试要。

## [0.3.41] - 2026-09-12

给下游网关补齐"迁移先决改造"四件事：握手契约、分通道健康与连接代次、跨重启的委托幂等、
带游标的回报回放。都是**新增**能力，老客户端不受影响；但 0.3.41 的客户端会要求服务端也是
0.3.41——握手里少了协议版本或启动代次一律 fail-closed，所以升级客户端必须同时同步 QMT 端。
